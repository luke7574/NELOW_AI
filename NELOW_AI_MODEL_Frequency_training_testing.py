...
AI_model_training = 'Frequency/AI_MODEL/NELOW_FREQ_model_2.h5'
AI_model_testing = 'Frequency/AI_MODEL/NELOW_FREQ_model_2.h5'

WAV_files_path_training = 'Frequency/Training/WAV_files/'
Numpy_files_path_training = 'Frequency/Training/Numpy_files/'

WAV_files_path_testing = 'Frequency/Testing/WAV_files/'
Numpy_files_path_testing = 'Frequency/Testing/Numpy_files/'
CSV_files_path_testing = 'Frequency/Testing/CSV_files/'

training_sound_preprocessing = 0   # 음성파일(wav) numpy배열로 변환하여 저장
model_training = 0
train_plot = 'C:/Users/gram/AI/NELOW/NELOW_AI/Frequency/plot_history/NELOW_FREQ_model_2.png'   #학습 그래프 경로/파일명 설정
...
if model_training:
    ...
    # kernel_size => 연속된 주파수값 계산할 계수 (너무 작으면 노이즈에 민감해질수있음)
    model.add(Conv1D(32, kernel_size=5, activation='linear', input_shape=(3000, 1), padding='same'))
    model.add(LeakyReLU(alpha=0.1)) # 활성함수 relu 변형함수로 음의 입력에 대해서도 작은 기울기를 제공
    model.add(MaxPooling1D(pool_size=2, padding='same'))
    model.add(Conv1D(64, kernel_size=5, activation='linear', padding='same'))
    model.add(LeakyReLU(alpha=0.1))
    model.add(MaxPooling1D(pool_size=2, padding='same'))
    model.add(Conv1D(128, kernel_size=5, activation='linear', padding='same'))
    model.add(LeakyReLU(alpha=0.1))
    model.add(MaxPooling1D(pool_size=2, padding='same'))
    model.add(Flatten())
